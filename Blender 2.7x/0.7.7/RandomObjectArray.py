#----------------------------------------------------
#Random Object Array by Manuel Geissinger
#----------------------------------------------------
bl_info = \
    {
...
class RandomObjectArraySettings(bpy.types.PropertyGroup):
    ...
    objList = bpy.props.StringProperty()
    ...
    autoLastActive = bpy.props.StringProperty()
...
class GroupCountData(bpy.types.PropertyGroup):
...
class ROA_update_grouplist(bpy.types.Operator):
    ...
    def execute(self, context):   
        ...
        if (isROA() or self.buttonPress) and bpy.data.groups:
            if bpy.context.scene.objGroup != "":
                """Add the master object first"""
                item = bpy.context.scene.groupCountData.add()
                item.name = bpy.context.active_object.name+" (Master)" 
                for ob in bpy.data.groups[bpy.context.scene.objGroup].objects:
                    item = bpy.context.scene.groupCountData.add()
                    item.name = ob.name
        return {'FINISHED'}             
...
def scene_update(scene):
    if active_obj_changed():
        ...
        if bpy.context.scene.roa_settings.autosave and isROA():
        ...
        if bpy.context.scene.is_updated: 
            if isROA(): 
                if checkAll():                    
                    ...
                    if len(bpy.context.scene.groupCountData) > 0 and bpy.context.scene.objGroup != "":
                        bpy.context.scene.groupCountData[bpy.context.scene.roa_settings.groupCountIndex].count = bpy.context.scene.roa_settings.groupActiveCount
...
class ROA_Update(bpy.types.Operator):
...
class ROA_SelectParent(bpy.types.Operator):
    ...
    def execute(self, context): 
        if bpy.context.active_object is not None and len(bpy.context.selected_objects) > 0:
            isROA = False
            try:
                isROA = bpy.context.active_object.parent["isROA"]
            except:
                pass
           
            if isROA and bpy.context.active_object.parent is not None:
                current = bpy.context.active_object
                bpy.context.scene.objects.active = current.parent
                current.parent.select = True                
                current.select = False
                
        return {'FINISHED'}  
        
class ROA_EDIT(bpy.types.Operator):
    ...
    def execute(self, context): 
        if bpy.context.active_object is not None and len(bpy.context.selected_objects) > 0:
            isROA = False
            try:
                isROA = bpy.context.active_object["isROA"]
            except:
                pass
...
class ROA_Apply(bpy.types.Operator):
    ...
    def execute(self, context):                             
        ...
        srcList = []
        grpModList = []
        if context.scene.roa_settings.pickGrp and bpy.data.groups:
        ...
        originalLayers = originalObj.layers
            ...
            try:
                ...
                for ob in bpy.context.selected_objects: #sorted(bpy.context.selected_objects, key=lambda ob: ob.name):
                    ...
                    if index < objCount-1 and not "ROA_RT_BOX_TEMPORARY_OBJECT" in ob.name:
                        ...
                        index += 1
            try:
                bpy.data.objects["ROA_RT_BOX_TEMPORARY_OBJECT"].select = True
            except:
                pass
            
            masterObj.select = False                     
            bpy.ops.object.delete(use_global=False)
            bpy.context.scene.objects.active = masterObj
            masterObj.select = True
...
class ROA_Group_Count(UIList):     
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        split = layout.split()
        split.label(str(item.name))
        split = layout.split()
        split.label(str(item.count))
...
class CustomPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if bpy.context.active_object is None  or len(bpy.context.selected_objects) == 0:
            row.enabled=False
        ...
        row.operator(ROA_EDIT.bl_idname, text = rtButtonText)
        row = col.row(align=True)
        row.operator(ROA_Apply.bl_idname, text = "Apply").isRT = False 
        ...
        if bpy.context.active_object is not None and len(bpy.context.selected_objects) > 0:
            ...
            if isROA:
                row = col.row(align=True)
                row.operator(ROA_SelectParent.bl_idname, text = "Select Parent ROA Object")
            
        split = box.split()
        col = split.column(align=True)
        col.operator(ROA_Reset.bl_idname, text = "Reset all values") 
        col.operator(ROA_Set.bl_idname, text = "Assign ROA") 
        ...
        split.operator(ROA_Update.bl_idname, text="Update Relatime Preview")
        ...
        if context.scene.roa_settings.pickGrp:
            ...
            col.template_list("ROA_Group_Count", "", context.scene, "groupCountData", context.scene.roa_settings, "groupCountIndex")
            split = box.split()
            col = split.column()             
            ...
            if context.scene.objGroup != "":
                ...
                col.operator(ROA_update_grouplist.bl_idname, text="Update group list").buttonPress = True
                col = split.column()  
...
def register():
    ...
    bpy.types.Scene.groupCountData = bpy.props.CollectionProperty(type=GroupCountData)
