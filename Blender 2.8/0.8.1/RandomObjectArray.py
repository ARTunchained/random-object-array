...
class RandomObjectArraySettings(bpy.types.PropertyGroup):
	...
	invert_roa_vertex_group_density : BoolProperty(name='invert density', description='invert vertex group density', default=False)
	invert_roa_vertex_group_scale : BoolProperty(name='invert scale', description='invert vertex group scale', default=False)
	...
	groupedDataIndex : IntProperty(default=0)
	groupedDataIndexLast : IntProperty(default=-1)
...
def checkAll():
	...
	check = check or stateChange("roa_targetObject", bpy.context.scene.roa_targetObject) or stateChange("roa_used_collection", bpy.context.scene.roa_used_collection) or stateChange("pickGrp", bpy.context.scene.roa_settings.pickGrp)
	...
	check = check or stateChange("roa_vertex_group_density", bpy.context.scene.roa_vertex_group_density) or stateChange("invert_roa_vertex_group_density", bpy.context.scene.roa_settings.invert_roa_vertex_group_density) 
...
stateDict = {"realtime": False,
			 ...
			 "target_mode": "FACES",
			 "cyclic": False,
			 "roa_vertex_group_density": "",
			 "invert_roa_vertex_group_density": False,
			 "autosave": True,
			 "groupActiveCount": -1,
			 "groupNoRepeat": False,
			 "rowoffset": False,
...
def _update_grouplist_():
	if len(bpy.context.scene.roa_groupedData) > 0:
		#clear list
		bpy.context.scene.roa_groupedData.clear()
		#also clear index
		bpy.context.scene.roa_settings.groupedDataIndex = 0
		bpy.context.scene.roa_settings.groupedDataIndexLast = -1
	#
	
	#if (isROA() or self.buttonPress) and bpy.data.collections: # FIXME: buttonPress removed
	if isROA(): # and bpy.data.collections:
		#
		collA = bpy.context.scene.roa_used_collection 
		# if collection is used: 
		if collA != "":
			"""Add the master object first"""
			item = bpy.context.scene.roa_groupedData.add()
			item.name = get_active_object().name+" (Master)" 
			for ob in bpy.data.collections[ collA ] .objects:
				item = bpy.context.scene.roa_groupedData.add()
				item.name = ob.name
			#for
		#if roa_used_collection
	#if isROA
...
def handler_depsgraph_update_pre(paramx):  # note: was scene_update() in blender 2.79
	...
	if active_obj_changed():
		...
		#
		# 2.8.CHANGE -- was: bpy.ops.object.roa_updategrouplist()
		_update_grouplist_()
		...
		if True:
			...
			if isROA(): 
				if checkAll():					
					...
					if len(scn.roa_groupedData) > 0 and scn.roa_used_collection != "":
						scn.roa_groupedData[ scn.roa_settings.groupedDataIndex ].count = scn.roa_settings.groupActiveCount
...
class OT_ROA_Reset(bpy.types.Operator):
	...
	def execute(self, context):
		...
		context.scene.roa_targetObject = stateDefault["roa_targetObject"]
		context.scene.roa_used_collection = stateDefault["roa_used_collection"] 
		...
		context.scene.roa_vertex_group_density = stateDefault["roa_vertex_group_density"]	
		context.scene.roa_settings.invert_roa_vertex_group_density = stateDefault["invert_roa_vertex_group_density"]	
...
class OT_ROA_Store(bpy.types.Operator):
	...
	def execute(self, context): 
		...
		originalObject["roa_targetObject"] = context.scene.roa_targetObject	  
		#72
		originalObject["roa_used_collection"] = context.scene.roa_used_collection
		...
		#79
		originalObject["roa_vertex_group_density"] = context.scene.roa_vertex_group_density
		originalObject["invert_roa_vertex_group_density"] = context.scene.roa_settings.invert_roa_vertex_group_density
		#80
		...
		counts = ""
		if len(bpy.context.scene.roa_groupedData) > 0:
			for item in bpy.context.scene.roa_groupedData:
	...
	def execute(self, context): 
		...
		if ver > 70:
			try:
				...
				context.scene.roa_targetObject = originalObject["roa_targetObject"]
		...
		if ver > 71:  
			try:
				context.scene.roa_used_collection = originalObject["roa_used_collection"] 
		...
		if ver > 78:  
			try:
				context.scene.roa_vertex_group_density = originalObject["roa_vertex_group_density"] 
				context.scene.roa_settings.invert_roa_vertex_group_density = originalObject["invert_roa_vertex_group_density"] 
			except:
				self.report({'ERROR'}, "Sorry, something went wrong. Please report this bug.")			 
		if ver > 79:  
			try:
		...
		if ver > 80:  
			...
			try:
				...
				if counts != "":
					...
					if len(bpy.context.scene.roa_groupedData) > 0 and len(countList) > 0:
						for item in bpy.context.scene.roa_groupedData:
...
class OT_ROA_Apply(bpy.types.Operator):
	...
	def execute(self, context):							 
		...
		grpModList = []
		#
		## old 2.79: if context.scene.roa_settings.pickGrp and bpy.data.collections:
		#
		if context.scene.roa_settings.pickGrp: # and bpy.data.collections:
			if context.scene.roa_used_collection != "":
				obID = 0
				"""Append Master object first because we're using group count now"""
				srcList.append(masterObj)
				for ob in bpy.data.collections[context.scene.roa_used_collection].objects:
					srcList.append(ob)
...
#		originalLayers = originalObj.layers			# FIXME ??? : layers removed in 2.80  // use collections instead ? 
		...
		targetPointList = []
		targetPolyList = []
		if (context.scene.roa_targetObject != "") and (context.scene.roa_settings.dmode != True):
			#Global / Local to be removed
			context.scene.roa_settings.global_mode = 'GLOBAL'
			targetObj = context.scene.objects[context.scene.roa_targetObject]
			if targetObj.type == 'MESH':
				targetObjMatrix = targetObj.matrix_world
				if context.scene.roa_settings.target_mode == "FACES":										
					#Weight maps
					if context.scene.roa_vertex_group_density != "":
						targetGrp = targetObj.vertex_groups[context.scene.roa_vertex_group_density].index
						for poly in targetObj.data.polygons:
							...
							if context.scene.roa_settings.invert_roa_vertex_group_density:
								if polyWeight < 0.001:
		...
		for index in range(1, objCount+1):			
			...
			if useGroup:
				...
				tmpGroupCountData = []
				for element in bpy.context.scene.roa_groupedData:
			...
			if cloneToMesh:
				...
				"""Vertex Group Scale"""
				if context.scene.roa_vertex_group_scale != "":
					targetGrp = targetObj.vertex_groups[context.scene.roa_vertex_group_scale].index
					...
					if context.scene.roa_settings.invert_roa_vertex_group_scale:	
			...
			duplicate.select_set( False ) # 2.8.CHANGE
			currentObj.select_set( False ) # 2.8.CHANGE
		...
		if not self.isRT and context.scene.roa_settings.singObj:
			...
			for ob in objList:
				...
				try:
					sce.objects.link(ob) 
					#ob.layers = originalLayers # FIXME ??? : layers removed in 2.80 // use collections instead ? 
					ob.select_set( True )
				except:
					pass
				if firstOb:
		...
		#
		bpy.context.view_layer.update()  # 2.8.CHANGE -- was scene.update()
		...
		if context.scene.roa_settings.singObj and not self.isRT:	
			...
			for ob in objList:				  
				...
				obSelectState = ob.select_get()  # 2.8.CHANGE -- possible Error: Object (input object) not in View Layer 'View Layer'! (add to collection ??)
...
class ROA_PT_Panel(bpy.types.Panel):
	...
	def draw(self, context):
		...
		if context.scene.roa_settings.mirror:  
			...
			row = box.row(align=True)
			row.label( text="Mirror probability:" )
			row = box.row(align=True)
		...
		if context.scene.roa_settings.pickGrp:
			...
			## doc 2.80: prop_search(data, property, search_data, search_property, text="", text_ctxt="", translate=True, icon='NONE') 
			## old 2.79: col.prop_search( context.scene, "objGroup", bpy.data, "groups", text="")						 
			col.prop_search( context.scene, "roa_used_collection", bpy.data, "collections", text="")						 
			
			col.template_list("ROA_UL_Group_Count", "", context.scene, "roa_groupedData", context.scene.roa_settings, "groupedDataIndex")
			split = box.split()
			col = split.column()			 
			"""monitor for index change to update count correctly"""
			if context.scene.roa_used_collection != "":
				if len(context.scene.roa_groupedData) > 0:
					if context.scene.roa_settings.groupedDataIndexLast != context.scene.roa_settings.groupedDataIndex:
						context.scene.roa_settings.groupActiveCount = context.scene.roa_groupedData[context.scene.roa_settings.groupedDataIndex].count
						context.scene.roa_settings.groupedDataIndexLast = context.scene.roa_settings.groupedDataIndex 
					#
				#
				#
				col.operator( OT_ROA_update_grouplist.bl_idname, text="Update group list").buttonPress = True  ## FIXME ? buttonPress not used !? 
				#
				col = split.column()  
		...
		if not context.scene.roa_settings.dmode:
			...
			col.prop_search(context.scene, "roa_targetObject", context.scene, "objects", text="")
			col = split.column(align=True)	 
			col.label(text="Clone to")
			col.prop(context.scene.roa_settings, "target_mode")	
			if context.scene.roa_targetObject != "":
				try:
					row = box.row()
					row.label(text="Target polygon count: "+str(len(bpy.data.objects[context.scene.roa_targetObject].data.polygons)))
				except:
					pass
				split = box.split()
				col = split.column() 
				...
				"""Weight maps"""	
				ob = context.scene.objects[context.scene.roa_targetObject]
				split = box.split()
				col = split.column(align=True)	 
				row = col.row(align=True)
				row.prop_search(context.scene, "roa_vertex_group_density", ob, "vertex_groups", text="Density")
				row.prop(context.scene.roa_settings,"invert_roa_vertex_group_density", text="", toggle=True, icon='ARROW_LEFTRIGHT')
				
				"""Scale influence"""
				row = col.row(align=True)
				row.prop_search(context.scene, "roa_vertex_group_scale", ob, "vertex_groups", text="Scale")
				row.prop(context.scene.roa_settings,"invert_roa_vertex_group_scale", text="", toggle=True, icon='ARROW_LEFTRIGHT')
...
def register():
	...
	bpy.types.Scene.roa_targetObject = bpy.props.StringProperty()
	bpy.types.Scene.roa_vertex_group_density = bpy.props.StringProperty()
	bpy.types.Scene.roa_vertex_group_scale = bpy.props.StringProperty()
	bpy.types.Scene.roa_used_collection = bpy.props.StringProperty()
	bpy.types.Scene.roa_groupedData = bpy.props.CollectionProperty(type=ROA_GroupCountData)
	#
...
def unregister():
	...
	del bpy.types.Scene.roa_targetObject
	del bpy.types.Scene.roa_vertex_group_density 
	del bpy.types.Scene.roa_vertex_group_scale
	del bpy.types.Scene.roa_used_collection 
	del bpy.types.Scene.roa_groupedData
	#
