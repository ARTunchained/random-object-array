...
def object_to_collection(obj, orgCol):
	""" link object to original collection """

	if obj.name in orgCol.objects:
		pass
	else:
		orgCol.objects.link(obj)
		
		""" Unlink object from "Pick-from-Group"-Collection """
		# Why won't this work????
		#if bpy.context.scene.roa_settings.pickGrp:
		#	if obj.name in bpy.data.collections[bpy.context.scene.objGroup].objects:
		#		currentActive = get_active_object()
		#		bpy.data.collections[bpy.context.scene.objGroup].objects.unlink(obj)
		#		set_active_object(currentActive)
...
#		originalLayers = originalObj.layers			# FIXME ??? : layers removed in 2.80
		...
		for index in range(1, objCount+1):			
			...
			if len(objList) < index:  
				...
				object_to_collection( duplicate, originalObj.users_collection[0])  
				...
				object_to_collection( duplicate, originalObj.users_collection[0])   # add to active collection (=> add to view layer)
